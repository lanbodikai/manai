...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button',{name:'Review selected actions',exact:true}).click();
    ...
    await page.getByRole('button',{name:'Download decision review',exact:true}).click();
    const exported=JSON.parse(await fs.readFile(await (await reviewDownload).path(),'utf8'));
    assert.equal(exported.status,'not-approved-not-executed');
    assert.equal(exported.verified_cash_savings,null);
    assert.deepEqual(exported.selection,table.selection);
