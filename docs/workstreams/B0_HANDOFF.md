...
## Latest update — connected CFO decision support

On baseline **788b5ad**, the user requested the missing Track 2 decision support and then asked B to reference the UI at localhost:3000. B inspected that running synthetic overview in a browser and reused its rounded cards, colored icons, large figures and thin spending bar in the real-source Decisions page. Three compact tiles now show sample reference-priced allocation by outcome, a specifically owned CPU pilot recommendation, and the cost/readiness of being wrong. Rounded headline dollars expand to source hours, rate, period and outcome amounts. The existing table, top Optimize selected action, risk popup and optional 20% goal remain. Duplicate outcome visualization was removed from the lower disclosure. No synthetic reference-page amounts were copied into real-source results.

The CPU recommendation explains its evidence and limits: completed zero-compute jobs supply a testable hypothesis, not CPU compatibility or proven recovery. Its whole cohort cannot meet the 20% goal. Per-task evidence status separates needs-testing from scenario-modeled-not-measured. The existing local CPU arithmetic now publishes its immutable calculated snapshot to the top tiles and review popup. Edits are marked unsaved everywhere; source-version mismatch and deselection invalidate the displayed snapshot. Modeled net benefit can be negative. Fallback cost is a scenario, not a maximum loss; research harm remains unpriced. Selected non-CPU actions never inherit the CPU estimate, and combined net savings remains unavailable.

The popup adds the financial review, owner/benchmark/rollback prerequisites, source links and **Download decision review**. decision-review.example.json includes source identity, deduplicated selection, selected action evidence, reference rate, and the last calculated CPU assumptions/results when available. It records not-approved-not-executed, null combined/verified savings and all scope caveats. It is not claims.json. Export and evidence remain available when the modeling backend fails. Proceed still submits only fix IDs to the existing proposed model_only route; downloaded local assumptions are not silently sent to A. No approval, pilot execution or billing verification is simulated.

Actual checks: **npm test 51/51 PASS (3.04s)**; **npm run build PASS** with TypeScript and live-bundle fixture exclusion (existing ~830 kB chunk warning); **npm run test:local -- tests/local/optimization.spec.ts tests/local/pilot.spec.ts 4/4 PASS (6.5s)**; **git diff --check PASS**. Browser coverage verifies source-outcome percentages, the pending-estimate state, review/export identity, last-calculation preservation after edits, explicit combined-estimate absence, limit breaches, retry/error behavior, and focus restoration from both dialog entry points. Manual browser inspection covered the reference on :3000 and changed real-source :3002 page at desktop and 390px, including the full-screen risk dialog. Focus restoration through asynchronous selection was corrected and tested. Source screenshots stay local/ignored. Builder self-review only; unrelated browser suites not rerun.

Live multi-fix modeling, CPU performance benchmarks, verified cash savings, MCP acceptance, submission claims validation and whole-application Compose integration remain **NOT RUN** for this change. A must agree/implement canonical financial inputs/results and endpoint identity; B then connects them. Workload/platform owners must provide actual benchmark and billing evidence before a measured or savings-verified state can be shown. Modified paths remain dashboard/ and this handoff.
