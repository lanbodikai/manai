...
Plan clarification v0.11. Active API v0.3 remains unchanged; the reviewed verifier now motivates a [complete proposed v0.4 contract and fixtures](../../contracts/proposals/v0.4/README.md). See the [review evidence](../VERIFIER_HANDOFF_REVIEW.md). This update covers API/fixtures and A/B handoff, not calculator implementation. B can continue its v0.3 work until coordinated adoption.
...
A owns the canonical downside analysis inside each immutable `Audit.downside`. It works without C or a model key. C reads that result and uses MCP evidence to explain or challenge it. B displays A's result even when C is absent. No extra endpoint, risk score, probability model or automatic remediation is needed. Quantified single-job CPU cost/delay belongs to A under the proposed v0.4 extension.

## A's implementation

The table below describes the existing qualitative fallback. For quantified inputs, units, formulas and output fields, use the linked v0.4 proposal after A/B agreement.
...
The v0.3 fallback remains qualitative downside with unknown money. The independent verifier supplied a tested cost/delay model, so the intended next A/B slice is now quantified single-job CPU scenarios under the proposed v0.4 contract. It distinguishes successful replacement, failed trial plus full GPU rerun, and extra validation, preserving losses and unknowns. Legacy downside money remains null; the structured extension carries each metric with its own meaning. Do not subtract a one-job scenario from cohort recovery or call it realized savings. A/B must agree the proposed contract before dependent implementation.
...
2. C fetches `GET /api/audits/{audit_id}` from A. Its context includes the full scenario, eligibility, recovery, `action`, `downside`, limitations and provenance, including `downside.cpu_pilot` after v0.4 adoption. Do not send only the headline dollars.
...
B reads `Audit.downside` directly and labels it as unmeasured or assumed as appropriate. After v0.4 adoption, show the selected job, CPU cost, signed net reference value and completion change separately from cohort recovery; null rates or delays stay unknown. Render null money as “Not quantified,” preserving the risk explanation and guardrails. Keep C's optional commentary separate from the canonical downside. Pending form edits do not relabel an old audit as a new calculation.
