...
def build_review(s, page, action, context):
    ...
    check('coverage', True if complete else None, f"Retrieved {len(rows)} of {page['total']} records in the selected scope. " + ('Complete scoped pagination.' if complete else 'Partial review; these records cannot certify the full cohort total.'))
    check('sample_arithmetic', all(close(c['gross_reference_usd'] - c['intervention_reference_usd'], c['net_reference_usd']) for row in rows for c in row['cases'].values()),
          'Retrieved per-job case values satisfy gross minus intervention equals net, before action-level fixed costs.')
    ...
    async with slots:
      try:
       async with asyncio.timeout(28):
        ...
        page={'total':len(rows),'items':rows[:100]}
        ...
        if key:
            ...
            if set(out)!={'selected_fact_ids'} or not isinstance(selected,list) or not 1<=len(selected)<=16 or any(not isinstance(x,str) or x not in allowed for x in selected) or len(set(selected))!=len(selected): raise ValueError('model selection')
      ...
      except Exception: raise ServiceError(502,'REVIEW_INVALID_RESPONSE','Review could not validate its evidence or model response. Existing results are unchanged.') from None
