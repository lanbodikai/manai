...
export function CostOptimization({ api, modelAvailable = true, onOpenAuditedPilot, portfolio }: { api: DashboardApi; modelAvailable?: boolean; onOpenAuditedPilot?: () => void; portfolio?:ModelState }) {
        ...
        <div className="decision-scroll" role="region" aria-label="Cost optimization decision table" tabIndex={0}>
          <table className="decision-table">
            ...
            <tbody>{actionableRows.map(row => {
              ...
              return <tr key={row.id} className={[selected.includes(row.id) ? "decision-selected" : "", !built ? "decision-unavailable" : ""].filter(Boolean).join(" ")}>
