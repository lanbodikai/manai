...
export function CpuPilotPlanner({source}: {source:PilotSource}) {
  ...
  return <section id="cpu-pilot-planner" tabIndex={-1} className="panel pilot-planner" aria-labelledby="pilot-heading">
    <div className="pilot-title"><div><span className="eyebrow">3 · COMPARE BENEFIT WITH DOWNSIDE</span><h2 id="pilot-heading">Should we run the CPU pilot?</h2><p>Completed jobs with zero recorded GPU compute are candidates for a test. CPU compatibility is unproven.</p></div><span className="badge">{source.synthetic ? "Synthetic planning example" : "Local scenario · not an audited result"}</span></div>
    <p className="pilot-scope">Scope: {number(source.cohortHours)} eligible GPU-hours. Owner: Platform + workload owner. This planner covers the CPU pilot only; other selected fixes are excluded from its benefit.</p>
    ...
    {!r && <div className="pilot-placeholder"><ShieldCheck size={22}/><p>Enter costs and performance assumptions to see the benefit range, break-even point and rollback decision. Blank costs remain unknown.</p></div>}
    {r && snapshot && <section className="pilot-results" aria-label="Calculated pilot result">
      ...
      <div className="pilot-scenario-grid">{r.scenarios.map(s => <article key={s.label}><h4>{s.label}</h4><strong>{usd(s.net)}</strong><dl><dt>GPU value avoided</dt><dd>{usd(s.avoided)}</dd><dt>CPU cost</dt><dd>{usd(s.cpuCost)}</dd><dt>Setup + reserve</dt><dd>{usd(r.overhead)}</dd><dt>Cost after change</dt><dd>{usd(s.afterCost)}</dd></dl></article>)}</div>
