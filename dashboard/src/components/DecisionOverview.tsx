...
export function DecisionOverview({totalHours,price,windowLabel,outcomes,hasOutcomes,cpu,review,onModel,onReview,disabled}:{
  ...
  return <section className="decision-overview" aria-label="CFO decision overview">
    ...
    <article className="panel decision-tile recommendation-tile">
      ...
      <h2>{cpu?.affected_jobs ? "Test cheaper processors" : "Investigate the evidence"}</h2>
      ...
      <div className="tile-actions"><button className="text-button" onClick={onModel} disabled={disabled || !cpu?.affected_jobs}>{r ? "Edit pilot estimate" : "Estimate this pilot"} <ArrowRight size={14}/></button></div>
    ...
    <article className="panel decision-tile downside-tile">
      ...
      <button className="text-button" onClick={onReview} disabled={disabled || !cpu?.affected_jobs}>Review pilot risks <ArrowRight size={14}/></button>
