...
export function DecisionOverview({totalHours,price,windowLabel,outcomes,hasOutcomes,cpu,review,onModel,onReview,disabled,hardware,hardwareAllocation="memory_adjusted"}:{
  ...
  return <section className="decision-overview" aria-label="CFO decision overview">
    ...
    <article className="panel decision-tile recommendation-tile">
      ...
      <div className="tile-actions"><button className="text-button" onClick={onModel} disabled={disabled || !cpu?.affected_jobs}>{h ? "Inspect hardware scenario" : r ? "Update pilot scenario" : "Model pilot scenario"} <ArrowRight size={14}/></button></div>
    ...
    <article className="panel decision-tile downside-tile">
      ...
      <h2>{h ? range(h.failure_extra_reference_usd.low,h.failure_extra_reference_usd.high) : r ? usd(r.extraSpend) : "Not yet priced"}</h2>
      <p>{h ? "Extra modeled cost if every fitting job needs a full GPU rerun" : r ? "Modeled extra cost if the CPU pilot falls back to GPUs" : "Estimate the downside before approving a change"}</p>
      <div className="decision-estimate"><span>Decision readiness</span><strong className="readiness-value">{h ? "Simulation only · owner review needed" : r ? r.spendBreached || r.slowdownBreached ? "Limits exceeded" : "Owner review needed" : "Pilot evidence needed"}</strong><small>{h ? "Failure doubles runtime in this model. Setup and research impact remain unpriced." : r ? "Further retries can cost more. Research impact remains unpriced." : "No measured cash savings yet"}</small></div>
