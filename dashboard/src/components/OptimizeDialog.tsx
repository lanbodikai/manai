...
export function OptimizeDialog({modelAvailable = true,rows,share,hours,jobs,overlap,sending,error,receipt,ready,review,restoreFocus,onDownload,onProceed,onReturn}:{
  ...
  return <dialog ref={dialog} className="optimize-dialog" aria-labelledby="optimize-dialog-title" aria-describedby="optimize-dialog-description" onKeyDown={e => {
    ...
    <header><div><span className="eyebrow">REVIEW BEFORE PROCEEDING</span><h2 id="optimize-dialog-title">{modelAvailable ? "Optimize" : "Review"} {rows.length} selected {rows.length===1 ? "task" : "tasks"}?</h2></div><button className="icon-button" aria-label="Return to decisions" onClick={onReturn}><X size={20}/></button></header>
