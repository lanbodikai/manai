...
describe("decision table interactions", () => {
  it("selects fixes, shows a deduplicated percentage, and preserves selection after a backend error", async () => {
    ...
    expect(within(cpuCard).getByText("reference value")).toBeVisible();
    expect(screen.getAllByRole("row")).toHaveLength(9);
    expect(screen.queryByLabelText("Recovery high (%)")).not.toBeInTheDocument();
    expect(screen.getByRole("button",{name:"Optimize selected"})).toBeDisabled();
    ...
    await waitFor(() => expect(screen.getByRole("button",{name:"Optimize selected"})).toBeEnabled());
    expect(screen.queryByText("What could go wrong?",{exact:true})).not.toBeInTheDocument();
    await user.click(screen.getByRole("button",{name:"Optimize selected"}));
    let popup=screen.getByRole("dialog",{name:"Optimize 2 selected tasks?"});
    ...
    await user.click(within(popup).getByRole("button",{name:"Proceed"}));
    expect(await screen.findByRole("alert")).toHaveTextContent("Backend unavailable");
    await user.click(within(popup).getByRole("button",{name:"Retry proceed"}));
  ...
  it("ignores an earlier selection summary arriving after the current summary", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button",{name:"Optimize selected"})).toBeEnabled());
    expect(screen.getByText("2 tasks selected",{exact:true})).toBeVisible();
  });
});
