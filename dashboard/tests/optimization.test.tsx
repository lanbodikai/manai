...
describe("decision table interactions", () => {
  it("selects fixes, shows a deduplicated percentage, and preserves selection after a backend error", async () => {
    ...
    expect(within(cpuCard).getByText("$0–$18.75")).toBeVisible();
    await user.clear(screen.getByLabelText("Reference price ($/GPU-hour)"));
    expect(await screen.findByRole("alert")).toHaveTextContent("Enter a nonnegative price");
    await user.type(screen.getByLabelText("Reference price ($/GPU-hour)"),"5");
    expect(within(cpuCard).getByText("$0–$37.5")).toBeVisible();
    ...
    expect(within(screen.getByRole("region",{name:"Optimization context"})).getByText("$0–$37.5")).toBeVisible();
