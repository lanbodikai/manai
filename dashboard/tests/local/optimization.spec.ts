...
test("real decision percentages, checkbox selection, evidence and actual unavailable action", async ({page,request}) => {
  ...
  await expect(page.getByRole("heading",{name:"Prioritize a guarded pilot",exact:true})).toBeVisible();
  ...
  await expect(page.getByRole("button",{name:"Review selected actions"})).toBeEnabled();
  ...
  const trigger=page.getByRole("button",{name:"Review selected actions"});
  await trigger.click();
  const popup=page.getByRole("dialog",{name:"Review 2 selected actions"});
  ...
  await popup.getByRole("button",{name:"Request modeling review",exact:true}).click();
