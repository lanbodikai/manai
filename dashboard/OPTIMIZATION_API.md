...
Pressing Model (accessible name Compare CPU pilot) opens the local planner; checkbox selection alone does not open it. Engineering assumptions and detailed cost breakdowns start collapsed. See src/pilot-model.ts and src/components/CpuPilotPlanner.tsx. This is user-authorized B planning arithmetic, not an adopted shared contract or A financial audit. The sample 20% target uses total recorded hours × pinned GPU price × 0.2; it is not a forecast of next-quarter spend.
...
The Decisions page now leads with three compact tiles: where reference-priced allocation went, why the CPU cohort is the first proposed pilot, and the modeled downside/decision readiness. Styling follows the existing localhost:3000 synthetic overview, but values come from the current decision/catalogue source. The CPU planner's last immutable calculation feeds both headline benefit/downside and the confirmation popup. Unsaved assumptions remain visibly separate. The 20% goal is optional; a CPU cohort exposure ceiling explains why this pilot alone does not meet it.

**Download decision review** exports decision-review.example.json for owner handoff, independently of backend availability. It preserves the displayed source version, selected actions and deduplicated exposure, reference price, evidence paths, and last calculated CPU snapshot (including an unsaved-input flag). Combined net savings and verified cash savings are null. This is a local planning example, not claims.json, a canonical audit or approval. No API schema or route changed. The financial review never extends CPU pilot estimates to other selected fixes. Measured/verified statuses require evidence not available from the current historical dataset alone.

Open `http://127.0.0.1:3002/#optimization` in explicit local dataset mode. The page presents eight suggested investigations: CPU placement, idle sessions, low GPU activity, imbalance across GPUs, memory sizing, failed array tasks, timeouts and unsuccessful jobs with no GPU compute. The compact table shows task name, reference value, percentage, one-line fix and evidence link. Details retains owner, source-hour/job counts and accounting caveats. Checkboxes select one or several fixes; the combined exposure counts each job once. The selection/savings summary cards are removed. Optimize selected above the table opens a popup showing only selected tasks and their risks, with optional safeguards and calculation details. Return or Escape closes without submitting; Proceed requests backend modeling. Keyboard focus stays in the popup and returns to its trigger on close. Narrow screens use a full-screen popup.

The 20% goal and storage-style outcome bar are optional disclosures beneath the tasks. No target input is needed to investigate an opportunity. The mutually exclusive bar measures recorded allocation by job outcome, not savings. Legend buttons provide plain-language explanations; categories must reconcile to the sample total. Narrow layouts stack the content.
...
**Optimize selected** opens the confirmation popup; only **Proceed** calls this route via the existing same-origin `/api` proxy. Return does not submit. Closing after submission does not cancel a request already sent. The local dataset middleware does not intercept it, acknowledge it or invent a result. Missing A returns an explicit error while retaining selections and access to evidence. No financial result is shown until implemented by A. HTTP acceptance alone will not count as successful optimization or live-data acceptance.
...
The frontend validates version/request/selection identity, accepts only `status=accepted`, rejects synthetic receipts in HTTP mode, and explicitly states that no workload change or savings has been verified after acceptance. The follow-up financial-result API/UI is not specified or implemented in this change. Retries for an unchanged selection reuse the same `client_request_id`; A must implement idempotency and reject that ID with different inputs. The client does not automatically retry. A 15-second timeout reports an unknown request status rather than claiming cancellation. Unmounting or changing the selection prevents late responses from appearing as the current result.
